...
from utilities import url_to_im, query_to_labels, save, save_and_query
...
def stick(image, sticker):
    ...
    result=np.zeros(image.shape, dtype=np.uint8)
    mask = np.mean(sticker, 2) == 255
    mask2 = np.mean(sticker, 2) != 255
...
class StickerGenerator:
    ...
    def make_sticker(self, label, title="", pixel_threshold=0.01, save_threshold=0.9):
        ...
        if prob[label] > save_threshold:
            save_url = sticker_directory + "\\" +str(label) + "\\" + title + str(pixel_threshold) + str(prob[label]) + ".png"
