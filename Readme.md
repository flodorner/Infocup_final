...
<b>Since the server for queries to the blackboc seems to be offline now and the program relied on label feedback from the server, the program is currently not working as intended and needs some slight modifications to do so again.
